...
function processFile(){
    ...
    for(var i=0; i < fileContent.length; i++){
        ...
        const idx = bayesClassesInfo.findIndex((elem)=>{return elem.classBName === splitContent[splitContent.length-1]});

        if(idx === -1){
            bayesClassesInfo.push({classBName: splitContent[splitContent.length-1], data: [splitContent.splice(0,splitContent.length-1)]})
        }else{
            bayesClassesInfo[idx].data.push(splitContent.splice(0,splitContent.length-1));
        }
    }
